...
function showDedicationText() { //seguidores
  ...
  if (!text) {
    text = `Sonrisa tierna, cálida y brillante, como esos rayitos que llevas en el cabello.
Ojos marrones, oscuros como el café,que transmiten una ternura… y una fortaleza especial.
Una personalidad alegre y dulce, pero también sensible, frágil, como si no pudieras contener tus emociones.
Eres una chica fantástica,con defectos… pero defectos que te hacen única y diferente,
con esa mezcla tuya de infidelidad y amor desbordante.
Y si alguna vez lo olvidas, si te cuesta ver lo valiosa que eres, espero que recuerdes que hay alguien aquí, que te aprecia de verdad, que se ríe contigo, que te molesta sin piedad, pero también te escucha sin filtros, incluso cuando tu vida parece un bucle sin final feliz.
No cambies tu fruncido de castor, ni esa forma tan peculiar de negar que alguien no te agrada.
Porque Aitana, así como eres, vales más de lo que a veces puedas pensar.`;  } else {
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "Con Amor, tu tumba de confianza.";
...
// Mostrar la imagen final después de X milisegundos
setTimeout(() => {
  const img = document.getElementById('foto-final');
  if (img) {
    img.classList.add('visible');
  }
}, 9000); // Cambia el tiempo si quieres que aparezca antes o después


// Imagen 2: AyA2.jpg a los 8 segundos
setTimeout(() => {
  const img = document.getElementById('foto-extra-1');
  if (img) img.classList.add('visible');
}, 10000);

// Imagen 3: AyA3.jpg a los 10 segundos
setTimeout(() => {
  const img = document.getElementById('foto-extra-2');
  if (img) img.classList.add('visible');
}, 11000);

// Imagen 4: AyA4.jpg a los 12 segundos
setTimeout(() => {
  const img = document.getElementById('foto-extra-3');
  if (img) img.classList.add('visible');
}, 8000);
